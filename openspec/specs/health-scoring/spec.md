...
Compute a per-project Red/Amber/Green (RAG) health score as a pure, deterministic, re-runnable query
over the latest analysis run's `Analysis`-kind findings — grouping by health `Area`, reducing to a
weighted 0-100 score, bucketing by configured RAG thresholds, applying worst-case-floor override
rules, and surfacing an auditable result (including a "Needs PM Review" flag driven by aggregate
finding confidence). All weights, mappings, thresholds, and override rules are external, validated,
swappable configuration. "RAG" here denotes the Red/Amber/Green health colour, never

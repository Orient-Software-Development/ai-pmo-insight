...
Route each LLM-backed agent (`RiskAndIssue`, `Narrative`, `Challenge`, `Review`) to an independently
configured provider and model through configuration alone, with no code change to any agent, prompt,
or the orchestrator. A single `RoutingLlmClient` — registered as the sole `ILlmClient` — dispatches
each `CompleteAsync` by `LlmRequest.SkillName` to a per-agent inner client built once at startup via
an `ILlmClientFactory`. Configuration is an `Llm.Default` block plus optional `Llm.Agents.<SkillName>`
overrides, backwards-compatible with the legacy flat `Llm.*` shape, and keeps API keys confidential.
...
#### Scenario: OpenAI selector constructs a working adapter

- **WHEN** the factory is invoked with `{ Provider: "openai", ModelId: "gpt-4o-mini", ApiKey: "K2" }`
- **THEN** it returns an adapter whose `CompleteAsync` performs a real OpenAI Chat Completions API call (no `NotImplementedException`), constructed with the supplied model and key
...
- **WHEN** any adapter surfaces a provider error

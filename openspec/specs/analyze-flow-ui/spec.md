# analyze-flow-ui Specification

## Purpose

Provide a dedicated, authenticated React SPA cold-start page (route `/upload`) that hosts the
upload → analyze flow — selecting a project export, uploading it (`POST /api/ingest/upload`), and
running analysis on the resulting upload (`POST /api/analyze/{uploadId}`) — extracted out of the
Level-2 project view and made the post-login landing route. The page is presentation-only over the
existing ingest and analyze surfaces: it preserves the accepted-format / CSV-rejection behaviour,
presents only what those endpoints return (an upload result, an analysis result, and a coarse
request-lifecycle pipeline status), and renders any richer wireframe panels the current API does not
back as explicit "not yet captured — follow-on" placeholders, introducing no change to ingest,

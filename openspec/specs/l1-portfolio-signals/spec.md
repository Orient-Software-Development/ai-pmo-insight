# l1-portfolio-signals Specification

## Purpose

Roll up the L1 (executive portfolio) signals that were placeholders — financial exposure, decision
backlog, key-person concentration, and a labelled customer-exposure proxy — from findings that now carry
the structured data to back them. This capability defines the rollup rules and rendering intent; it does
not define the underlying agents/metrics it consumes (financial-exposure metric, Decision agent findings)
except for key-person concentration, which this change's Resource agent produces.

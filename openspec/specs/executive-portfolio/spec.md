# executive-portfolio Specification

## Purpose

Provide the Level-1 executive portfolio roll-up: portfolio-wide discovery of the distinct project keys
present in the findings store, a roll-up that scores each project via the existing per-project
health-scoring service (its latest run, no re-analysis or LLM cost) and aggregates the results (RAG bucket
counts, "Needs PM Review" count, aggregate confidence, and a worst-first intervention list with cited
reasons), an authenticated view-only `GET /api/portfolio` read endpoint, and the Level-1 React executive
view that renders the backed panels from live data while flagging panels the current finding shape cannot
back. This capability introduces a real backend discovery slice but requires no change to agents, prompts,
finding shape, or existing API contracts.
...
cannot back — per-decision days-open/owner detail and owned/dated recommended actions — SHALL render a
clear "not yet captured" follow-on state and SHALL NOT display fabricated values. This view SHALL require
no change to agents, prompts, finding shape, or existing API contracts.
...
- **WHEN** the v2 layout includes a per-decision-detail or owned-recommendations panel that the roll-up
  cannot populate

...
Presentation-only React SPA auth surfaces: sign-in and register on one card at `/login`, an authenticated
change-password page, and the signed-in disclosure user-menu. These surfaces are built over the existing
`/api/auth/*` endpoints with no change to auth contracts, cookies, JWT, ASP.NET Identity, or the

...
| Schema (create/alter tables) | EF Core **migrations** (`source/AiPMOInsight.Infrastructure/Migrations`) |
...
in `AuthCookies` (`AiPMOInsight.Api/Security`).

- **Issuing** — `ITokenService` / `TokenService` (`AiPMOInsight.Api/Security`) signs an HS256 JWT with
...
dotnet run --project source/AiPMOInsight.Api     # Development → auto-migrates + seeds roles/admin
```

Default local connection (`appsettings.json`):
```
Host=localhost;Port=5432;Database=aipmoinsight;Username=aipmoinsight;Password=aipmoinsight
```
...
Inspect:
```bash
docker exec aipmoinsight-postgres psql -U aipmoinsight -d aipmoinsight -c "\dt"   # AspNet* + widgets
```
...
```bash
dotnet ef migrations add <Name> --project source/AiPMOInsight.Infrastructure --startup-project source/AiPMOInsight.Api
dotnet ef database update       --project source/AiPMOInsight.Infrastructure --startup-project source/AiPMOInsight.Api
```
...
1. `AiPMOInsight.Infrastructure` — swap `Npgsql.EntityFrameworkCore.PostgreSQL` for the matching EF

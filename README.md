...
- **Per-agent LLM routing** — the four LLM-backed agents each pick their own provider/model via `Llm.Default` + `Llm.Agents.<SkillName>` config. Selectors: `fake` (fixture responses, no API key — demo/tests), `anthropic`, `openai` (stubs pending real HTTP wiring — deliberate follow-up). Provider swap is a config change; no agent, prompt, or orchestrator code moves.

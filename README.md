...
- **CQRS** via a lightweight in-process mediator (`AiPMOInsight.Application/Messaging`) — no MediatR.
...
- **Optional React + Vite SPA** — `source/AiPMOInsight.Api/ClientApp`, toggled via template parameter.
...
dotnet run --project source/AiPMOInsight.Api    # serves /health, /api/widgets, /openapi/v1.json
...
> `dotnet run --project source/AiPMOInsight.Api`; `/` redirects to `/openapi/v1.json`.
...
| `source/AiPMOInsight.Api/ClientApp/` | React + Vite SPA (excluded when `--client-framework none`) |
| `tests/<Name>.Tests/` | Test projects (xUnit + AwesomeAssertions) |
| `AiPMOInsight.sln` | Solution file |
...
Schema is owned by EF Core migrations (`source/AiPMOInsight.Infrastructure/Migrations`).
...
`AiPMOInsight` is renamed to your chosen project name throughout.

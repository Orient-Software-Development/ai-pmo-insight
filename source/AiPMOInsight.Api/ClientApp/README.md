...
Vite + React 19 single-page app for the AiPMOInsight service.

## Develop

```bash
# 1. Run the API (separate terminal) — serves https://localhost:7443
dotnet run --project ..      # from source/AiPMOInsight.Api
...
On `dotnet publish` of `AiPMOInsight.Api`, this build runs automatically and its
output is copied into `wwwroot` (see `AiPMOInsight.Api.csproj`).
...
The aipmoinsight pages use `fetch` directly so this step is optional.

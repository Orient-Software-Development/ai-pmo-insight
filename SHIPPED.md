...
Historical log of shipped features and change decisions. Each `>-quoted` block was originally in
`CLAUDE.md` alongside the shipped change; moved here so `CLAUDE.md` stays lean and focused on
current-state guidance. Chronological, newest last. Load into context on demand (e.g. "read
SHIPPED.md's Auth block") rather than every session.
...
> **Spec-drift sensor (Layer 1 static, `feat/spec-drift`):** the API's live OpenAPI document
> (`/openapi/v1.json`, generated by `Microsoft.AspNetCore.OpenApi`'s `AddOpenApi()`/`MapOpenApi()`) is
> pinned by the committed baseline at repo-root `openapi.json`. Endpoints declare their response
> shape via `.Produces<T>(StatusCodes.Status200OK)` (+ `.Produces(404)` on the nullable → 404 patterns)
> — without this, minimal APIs returning `Results.Ok(...)` (opaque `IResult`) would leave the
> generator with **no response schema at all**, and the baseline would only catch request-side +
> path drift. A schema-name transformer on `AddOpenApi()` disambiguates the many nested `Result`
> records (`ScorePortfolio.Result`, `ScoreProject.Result`, …) by qualifying with their declaring
> type (`ScorePortfolioResult`, `ScoreProjectResult`) — otherwise they all collide on the name
> "Result" in `components.schemas`. The xUnit test `OpenApiDriftTest`
> (`tests/AiPMOInsight.Api.Tests/OpenApiDriftTest.cs`) boots the API via `TestWebAppFactory`, GETs
> the live doc, and **structurally** compares it to the baseline (parsed with `JsonDocument`;
> objects compared with keys sorted, arrays positionally — so key-order non-determinism doesn't
> flap, but real drift fails). Any added / removed / renamed endpoint, parameter, or DTO field
> fails the build so the baseline update is reviewed in the same PR as the code. To accept a
> deliberate API change, regenerate with `UPDATE_OPENAPI_BASELINE=1 dotnet test --filter
> FullyQualifiedName~OpenApiDriftTest` — the baseline is rewritten with **sorted keys + indented
> formatting** (stable across regenerations, regardless of the generator's internal key order) —
> and commit the diff. Baseline lookup walks up from `AppContext.BaseDirectory` to the first
> `.git` folder, so the READ and the UPDATE hit the same source file (no copy-to-output
> indirection). CI wiring: `.github/workflows/ci.yml` (`dotnet restore` → `build -c Release` →
...
> the spec declares for the actual returned status code, and validates the body against it. Path
> params are substituted with type-aware placeholders (a fixed GUID for `format:uuid`, a fixed
> string otherwise) — deterministic, one call per endpoint; no fuzzing. Since the in-memory EF
> store is empty, most typed-lookup endpoints (`/api/uploads/{id}/findings`,
> `/api/projects/{key}/health`, `/api/projects/{key}/progress`) return **404**, which requires the
> endpoint to have declared `.Produces(404)` — otherwise the test flags an *undeclared status*
> and fails. **POST endpoints are out of scope**: generating valid request bodies per schema
...
> No CI wiring beyond Layer 1's — the runtime test is another `[Fact]` in the same test project,
> picked up by the same `dotnet test` step. Layer 3 (LLM semantic drift over `CLAUDE.md`
> invariants) is not built until Layers 1+2 prove insufficient in practice. The nswag TypeScript
> client generator (`ClientApp/nswag.json` → `src/web-api-client.ts`) is **configured but never
> adopted** — no `web-api-client.ts` exists, the React app calls `/api/*` via hand-written
> `fetch()` from 8 files. Client regeneration is therefore not a follow-on step here; migrating
> the React app to consume a generated client would be a separate change.
